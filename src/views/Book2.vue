...
const formSchema = toTypedSchema(
  z.object({
    ...
    outfit_change: z.string().optional(),
...
const onSubmit = handleSubmit((values) => {
  http
    ...
    .catch((err) => {
      ...
      notify({
        text: err.response.data.message || "Booking successful!",
        duration: 10000,
        speed: 1000,
        type: "error",
      });
    ...
    <div class="px-40">
      <form @submit="onSubmit" class="grid grid-cols-2 gap-4">
        ...
        <div class="space-y-4">
          ...
          <div v-show="values.event_type == 'Photo Shoot'">
            <FormField name="event" v-slot="{ componentField }">
              <FormItem>
                <FormLabel>Type of Photo Shoot</FormLabel>
                <Select v-bind="componentField">
                  <SelectTrigger class="w-full rounded">
                  ...
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel>Type of Photo Shoot</SelectLabel>
                      <SelectItem
                        v-for="value in photoshoot_events"
                        :key="value.value"
                        :value="value.value"
                      >
                        {{ value.text }}
          ...
          <div v-show="values.event_type == 'Video Shoot'">
            <FormField name="event" v-slot="{ componentField }">
              <FormItem>
                <FormLabel>Type of Video Shoot</FormLabel>
                <Select v-bind="componentField">
                  <SelectTrigger class="w-full rounded">
                    <SelectValue placeholder="Select the type of shoot" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                      <SelectLabel>Type of Video Shoot</SelectLabel>
                      <SelectItem
                        v-for="value in videoshoot_events"
                        :key="value.value"
                        :value="value.value"
                      >
                        {{ value.text }}
          ...
          <div
            v-show="
              values.event_type == 'Photo Shoot' &&
              (values.event == 'Studio Shoot' ||
                values.event == 'Outdoor Shoot')
            "
          >
          ...
          <div
            v-show="
              ...
              (values.event == 'Studio Shoot' ||
                values.event == 'Outdoor Shoot') &&
              values.outfit_change == 'Yes'
            "
          >
            <FormField
              name="number_of_outfit_changes"
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input
                    ...
                    type="number"
                    min="1"
                    placeholder="Enter the number of outfit changes"
                    v-bind="componentField"
                  />
                </FormControl>
                <FormMessage />
        ...
        <div class="flex justify-center col-span-2">
          <Button
            type="submit"
            class="w-1/2 rounded bg-green-500 hover:bg-green-600 text-white"
          >
            Submit
          </Button>
        </div>
